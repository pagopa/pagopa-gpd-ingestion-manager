Feature: All about ingestion events consumed by Azure functions gpd-ingestion-manager
  ...
  Scenario: CRUD operations on payment position table in GPD database are published into data lake event hub
    Given a create operation on payment position table with id '11111111' and company name 'SkyLab Inc.' in GPD database
  ...
  Scenario: CRUD operations on payment option table in GPD database are published into data lake event hub
    ...
    And an update operation on field description with new value 'Targa AN09WQS' on the same payment option in GPD database
    ...
    And the payment option update operation has the description updated and anonymized
  ...
  Scenario: CRUD operations on transfer table in GPD database are published into data lake event hub
    ...
    And a create operation on transfer table with id '32111111' and category '9/0101108TS/' and remittanceInformation 'Targa AN09WQS' and associated to payment option with id 32222222 in GPD database
    ...
    And the operations have the remittanceInformation anonimized