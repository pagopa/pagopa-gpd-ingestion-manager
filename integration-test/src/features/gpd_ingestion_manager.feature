Feature: All about ingestion events consumed by Azure functions gpd-ingestion-manager
  ...
  Scenario: CRUD operations on payment position table in GPD database are published into data lake event hub
    ...
    And the payment position operations have id 11111121
  ...
  Scenario: CRUD operations on payment option table in GPD database are published into data lake event hub
    Given a payment position with id '11111121' and fiscal code '77777777777' and company name 'SkyLab Inc.' in GPD database
    And a create operation on payment option table with id '11111121' and description 'Canone Unico Patrimoniale - SkyLab Inc.' and associated to payment position with id 11111121 in GPD database
    ...
    Then the data lake topic returns the payment option 'create' operation with id '11111121-c'
    And the data lake topic returns the payment option 'update' operation with id '11111121-u'
    And the data lake topic returns the payment option 'delete' operation with id '11111121-d'
    And the payment option operations have id 11111121
