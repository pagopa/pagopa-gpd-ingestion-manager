Feature: All about ingestion events consumed by Azure functions gpd-ingestion-manager
  ...
  Scenario: CRUD operations on transfer table in GPD database are published into data lake event hub
    ...
    And a create operation on transfer table with id '32111111' and company name 'SkyLab Inc.' and associated to payment option with id 32222222 in GPD database
    And an update operation on field company name with new value 'Updated company name' on the same transfer in GPD database
    And a delete operation on the same transfer in GPD database
    When the transfer operations have been properly published on data lake event hub after 10000 ms
    ...
    And the transfer update operation has the company name updated